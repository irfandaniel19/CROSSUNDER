...
── PENDING / RECOMMENDED ADDITIONS ──────────────────────────────────

  ⬜ Supplier CRUD (add/edit/remove suppliers via admin panel)
  ⬜ Customer profile edit page (update CUST_NAME, CUST_NOPHONE)
  ⬜ Customer order history view page
  ⬜ Image file upload handler (currently uses filename only)
  ⬜ Pagination for large product/order lists
  ⬜ Email notification on order status change
  ⬜ Password reset / forgot password flow
  ⬜ Sales report export (CSV / PDF)
